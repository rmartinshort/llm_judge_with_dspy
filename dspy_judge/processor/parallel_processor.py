...
from ..llm_caller import (
    OpenAIStructuredOutputCaller, OpenAITextOutputCaller,
    AnthropicStructuredOutputCaller, AnthropicTextOutputCaller, 
    GeminiStructuredOutputCaller, GeminiTextOutputCaller
)
import logging
    ...
    try:
        ...
        if hasattr(metadata, 'id') and hasattr(metadata, 'usage'):
            return {
                ...
                "usage": {
                    "prompt_tokens": metadata.usage.prompt_tokens if metadata.usage else 0,
                    "completion_tokens": metadata.usage.completion_tokens if metadata.usage else 0,
                    "total_tokens": metadata.usage.total_tokens if metadata.usage else 0
                },
                "finish_reason": metadata.choices[0].finish_reason if metadata.choices else None,
        ...
        return {"error": f"Failed to serialize metadata: {str(e)}", "type": str(type(metadata))}
...
def _initialize_worker_dspy_from_signature(model_name: str, api_key: str, signature_docstring: str, signature_fields: Dict[str, Any], demos: list, temperature: float = 0):
    ...
    for field_name, field_info in signature_fields.items():
        ...
        if annotation_str == 'str':
            annotation = str
        elif annotation_str == 'bool':
            annotation = bool
        elif annotation_str == 'int':
            annotation = int
        elif annotation_str == 'float':
            annotation = float
        else:
...
def _process_single_example(
    ...
    """
    (example, system_prompt, model_name, input_field, temperature, max_tokens, response_template) = args
...
def _process_single_example_dspy(
    args: tuple
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        example.update({
            "dspy_response": response_dict,
            "dspy_metadata": usage_metadata
        })
    except Exception as e:
        example.update({
            "dspy_response": None,
            "dspy_metadata": {"error": str(e)}
        })
...
class ParallelProcessor:
    ...
    def __init__(self, llm_caller: Optional[LLMCallerBase] = None, max_workers: int = 4):
        """
    ...
    def _extract_signature_info_from_module(self, dspy_module) -> Dict[str, Any]:
        ...
        for name, field in signature.model_fields.items():
            ...
            field_info = {
                ...
                'field_type': field.json_schema_extra.get('__dspy_field_type', 'output'),
    ...
    def process_dataset(
        ...
        args_list = [
            (example, system_prompt, model_name, input_field, temperature, max_tokens, response_template)
    ...
    def process_dataset_with_dspy(
        ...
        print(f"Processing {len(dataset)} examples with {num_proc} workers using DSPy...")
    ...
    def get_token_statistics(self, processed_dataset: Dataset) -> Dict[str, Any]:
        ...
        """
        input_tokens = [ex["input_tokens"] for ex in processed_dataset if ex["input_tokens"] > 0]
        output_tokens = [ex["output_tokens"] for ex in processed_dataset if ex["output_tokens"] > 0]
